...
def alpha_schedule(time_step, N=100, att_1 = 0.99999, att_T = 0.000009, ctt_1 = 0.000009, ctt_T = 0.99999):
    ...
    if time_step == 1:
        # 一步扩散的特殊处理：直接从数据到噪声
        # at: 单步转移概率 = att_T（直接到最终噪声水平）
        # ct: 单步mask概率 = ctt_T（直接到最终mask概率）
        # bt: 剩余概率分配给均匀分布
        at = np.array([att_T], dtype=np.float64)
        ct = np.array([ctt_T], dtype=np.float64)
        bt = np.array([(1 - att_T - ctt_T) / N], dtype=np.float64)

        # 累积概率就是单步概率
        att = np.array([1.0, att_T], dtype=np.float64)  # [α̃_0, α̃_1]
        ctt = np.array([0.0, ctt_T], dtype=np.float64)  # [c̃_0, c̃_1]
        btt = np.array([(1 - att_T - ctt_T) / N], dtype=np.float64)

        return at, bt, ct, att[1:], btt, ctt[1:]
...
class DiffusionTransformer(nn.Module):
    ...
    def update_n_sample(self):
        # 获取总token数（序列长度）
        total_tokens = self.content_seq_len
        ...
        if self.num_timesteps == 100:
            ...
            # 自适应采样策略：根据总token数和扩散步数计算
            # 使用渐进式采样：早期采样少，中期稳定，后期加速
            n = self.num_timesteps

            if n == 1:
                # 1步去噪：一次性采样所有token
                self.n_sample = [total_tokens]
            elif n == 5:
                # 5步去噪：[1, 2, 5, 10, 15] 渐进加速
                self.n_sample = [1, 2, 5, 10, min(15, total_tokens)]
            elif n == 10:
                # 10步去噪：更平滑的渐进
                base = max(1, total_tokens // 20)  # 大约5%的token
                self.n_sample = [base] * n
                # 逐步增加采样数
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1) // 2)
            elif n == 15:
                # 15步去噪
                base = max(1, total_tokens // 30)
                self.n_sample = [base] * n
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1) // 2)
            elif n == 20:
                # 20步去噪
                base = max(1, total_tokens // 40)
                self.n_sample = [base] * n
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1) // 2)
            elif n == 50:
                # 50步去噪：更细粒度的渐进
                base = max(1, total_tokens // 100)
                self.n_sample = [base] * n
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1) // 2)
            elif n == 75:
                # 75步去噪
                base = max(1, total_tokens // 150)
                self.n_sample = [base] * n
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1) // 2)
            elif n == 100:
                # 100步去噪：使用原始策略但按比例缩放
                if self.prior_ps <= 10:
                    original = [1, 6] + [11, 10, 10] * 32 + [11, 15]
                else:
                    original = [1, 10] + [11, 10, 10] * 32 + [11, 11]
                # 按比例缩放以适应总token数
                scale_factor = total_tokens / 1024.0  # 原始假设1024个token
                self.n_sample = [max(1, int(x * scale_factor)) for x in original]
            else:
                # 通用策略：线性增加采样数
                base = max(1, total_tokens // (n * 2))
                self.n_sample = [base] * n
                for i in range(n):
                    self.n_sample[i] = min(total_tokens, base * (i + 1))
    ...
    def sample(
        ...
        if input['condition_embed_token'] != None:
            batch_size = input['condition_embed_token'].shape[0]
        else:
